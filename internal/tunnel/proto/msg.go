...
type AuthResp struct {
	// SessionID is the agent's session ID
	SessionID string
...
type BindResp struct {
	// EndpointID is the ID of the endpoint this bind will create.
	// This ID may be returned even when the endpoint isn't created if there is
	// an error of some sort, such as a conflict, later down the line.
	EndpointID string
	// TunnelID is the ID of the tunnel this bind created.
	TunnelID string
	// ClientID is a session-unique bind ID generated by the client, if empty, one is generated
	ClientID string `json:"Id"`
	// URL is the public URL of the bind (a human friendly combination of Hostname/Port/Proto/Opts)
	URL string
	// Proto is the protocol bound on
	Proto string
	// Opts are protocol-specific options that were chosen
	Opts any
	// Error is the error message if the server failed to bind
	Error string
	// Extra contains application-defined extra values
	Extra BindRespExtra
}
